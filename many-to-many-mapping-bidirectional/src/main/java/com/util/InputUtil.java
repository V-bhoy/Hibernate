...
public class InputUtil {
    public static int menuOptions(Scanner sc) {
        ...
        System.out.println("11. List student details using course Id");
        System.out.println("12. List all details using course Id");
        System.out.println("13. Register course");
        System.out.println("14. Register students in a course");
        System.out.println("15. Remove students in a course");
    ...
    public static Set<Integer> acceptStudentIds(Scanner sc){
        ...
        if(sc.hasNextInt()) {
            ...
            if(n < 0) {
                ...
                return acceptStudentIds(sc);
            }
            else{
                for(int i = 0; i < n; i++) {
                    while(true){
                        System.out.println("Enter valid student ID grater than 0: ");
                        if(sc.hasNextInt()) {
                            int id = sc.nextInt();
                            sc.nextLine();
                            if(id <= 0) {
    ...
    public static List<Student> acceptStudentsToDelete(Scanner sc, List<Student> students){
        List<Student> studentsDeleted = new ArrayList<>();
        for(Student student : students) {
            ...
            sc.nextLine();
            if(input == 'Y') {
                studentsDeleted.add(student);
            }
        }
        if(studentsDeleted.size() == students.size()) {
            System.out.println("Are you sure you want to delete all the students in the course? (y/n)");
            char input = sc.next().toUpperCase().charAt(0);
            sc.nextLine();
            if(input == 'N') {
